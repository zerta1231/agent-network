...
class EvoMapClient {
  ...
  // Search for agents/capsules on EvoMap network
  async discoverAgents(query = '') {
    try {
      // Try the nodes endpoint first
      const nodesResponse = await this.request('/a2a/nodes');
      if (nodesResponse && nodesResponse.nodes && nodesResponse.nodes.length > 0) {
        return nodesResponse.nodes.map(n => ({
          id: n.node_id || n.id,
          name: n.name || n.metadata?.name || 'Agent',
          type: 'evomap',
      ...
      // Try the fetch endpoint with capsule query
      ...
      const result = await this.request('/a2a/fetch', 'POST', envelope);
      const capsules = result.payload?.results || [];
      return capsules.map(c => ({
        id: c.id || c.capsule_id,
        name: c.name || c.title,
        type: 'evomap',
      ...
      console.log('EvoMap discover error:', e.message);
      return []; 
    }
  }
}
